...
def parse_smart_cookie(c_in):
    ...
    # JSON format detect (array ya object)
    try:
        ...
        if isinstance(data, list):
            for c in data:
                if isinstance(c, dict) and c.get("name") == "NetflixId":
                    return urllib.parse.unquote(c.get("value"))

        elif isinstance(data, dict):
            if "NetflixId" in data:
                return urllib.parse.unquote(data["NetflixId"])

    except:
        pass

    # Netscape browser cookie format detect
    match = re.search(r"NetflixId\s+([^\s]+)", c_in)
    if match:
        return urllib.parse.unquote(match.group(1))

    # Normal format detect
